...
int main() {
	...
	vector<string> onnx_names = {"test1",
								"test2",
								"test3",
								"test3v2",
								"test4",
	...
	for(string name : onnx_names){
		cout << "setting up: " << "/home/pi/Jeremy/yolo/majic/majic_onnx/" + name + ".onnx" << endl;
		
		cv::dnn::Net net = cv::dnn::readNet("/home/pi/Jeremy/yolo/majic/majic_onnx/" + name + ".onnx");
	...
	int previous_selected_net = 0;
	int selected_net = 0;
	int max_selected_net = nets.size();
	
	printf("max_selected_net = %d\n", max_selected_net);
	
	
	cv::dnn::Net net = nets[selected_net];
	...
	while(true){
		...
		// switch nets if needed
		if(previous_selected_net != selected_net){
			// user selected new net
			previous_selected_net = selected_net;
			
			net = nets[selected_net];
		}
		...
		if(input == 'q'){
			...
			selected_net--;
			if(selected_net < 0){
				selected_net = max_selected_net - 1;
			}
			
			//printf("\n\n--------------\nNEW SELECTED NET = %s\n--------------\n", onnx_names[selected_net]);
			cout << "\n\n---------------\n" << "NEW SELECTED NET = " << onnx_names[selected_net] << "\n---------------\n";
		}
		else if(input == '2'){
			selected_net++;
			if(selected_net >= max_selected_net){
				selected_net = 0;
			}
